...
function isActive(href) {
  ...
  if (navEl) {
    ...
    navEl.innerHTML = [
      '<div class="nav-inner">',
        '<a href="/" class="nav-logo" aria-label="DoctoraLáser — Inicio">',
          'Doctora Láser',
          '<em>Dra. Ana Vinasco · Bogotá</em>',
        '</a>',
        '<ul class="nav-links" id="nav-mobile" role="list">',
          navItem('/',               'Inicio'),
          navItem('/beneficios/',    'Beneficios'),
          navItem('/tratamientos/',  'Tratamientos'),
          navItem('/evidencia/',     'Evidencia'),
          navItem('/casos/',         'Casos'),
          '<li class="nav-cta-mobile">',
            '<a href="/agenda/" class="btn btn--wa" style="width:100%;justify-content:center">',
              '<i class="ti ti-brand-whatsapp" aria-hidden="true"></i> Agendar cita',
            '</a>',
          '</li>',
        '</ul>',
        '<a href="/agenda/" class="btn btn--primary nav-cta desktop-only">',
